(ns veterinary.phase
  "Phase 0->3 staged rollout -- the veterinary analog of
  ...
  performs; it is always a human veterinarian's call. `veterinary.
  governor`'s `:actuation/administer-treatment` high-stakes gate
  enforces the same invariant independently -- two layers, not one,
  agree on this. `:credential/screen` is likewise never auto-eligible,
  at any phase -- the same posture every sibling's KYC/conflict/
  ...
  `wagering.phase`, phase 3's `:auto` set here has only ONE member
  (`:case/intake`) -- this domain has no separate no-capital-risk
  'file' lifecycle distinct from the case itself.")

(def read-ops  #{})
(def write-ops #{:case/intake :jurisdiction/assess :credential/screen
...
(def phases
  ...
  {0 {:label "read-only"       :writes #{}                                                          :auto #{}}
   ...
   2 {:label "assisted-assess" :writes #{:case/intake :jurisdiction/assess :credential/screen}       :auto #{}}
...
(defn verdict->disposition
  "Map a Veterinary Care Governor verdict to a base disposition before
  the phase gate."
  [verdict]
